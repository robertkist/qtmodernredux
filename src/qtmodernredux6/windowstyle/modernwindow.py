...
from qtmodernredux6.windowstyle.windowresizer import Resizer
from qtmodernredux6.windowstyle.windowtitlelabel import WindowTitleLabel
from qtmodernredux6.windowstyle.windowframe import WindowFrame
from qtmodernredux6.windowstyle.tabfilter import TabFilter
...
class ModernWindow(QDialog):
    ...
    def __init__(self,
        ...
        def get_window_frame_widget(window_buttons_position: str) -> WindowFrame:
            ...
            if not isinstance(window, QDIALOG_TYPES):
                window_frame_widget.double_clicked.connect(self.__on_title_bar_double_clicked)
            ...
            if self.__title_bar:  # add optional titlebar
                ...
                self.__title_widget: Union[WindowTitleLabel, QWidget] = WindowTitleLabel(text='',
    ...
    def resizeEvent(self, _: QResizeEvent) -> None:
        ...
        if self.__tab_widget is not None:
            ...
            rect = QRect(0, 0, width + rounder_corner_px, self.__titlebar_height)
            self.__tab_widget.tabBar().setMask(rect)
        ...
        * ensures window is centered on screen or relative to their parent window
        ...
        if CENTER_MAINWINDOW:
            if parent is None and self.__center_window_first_time:  # type: ignore
                self.move(QGuiApplication.screenAt(QCursor.pos()).geometry().center() - self.rect().center())  # type: ignore
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if self.__mouse_pressed and self.__drag_move_enabled:
            ...
            if self.__mouse_pos is not None and self.__window_pos is not None:
    ...
    def __adjust_title_tabwidget(self, tab_widget: Union[QWidget, QTabWidget]) -> None:
        """
        ...
        if isinstance(tab_widget, QTabWidget):
            ...
            tab_widget.setProperty("class", "TitleTabWidget")
            tab_widget.tabBar().setProperty("class", "TitleTabBar")
            ...
            margin_top_px: int = self.__style.window.TITLE_BAR_TOP_MARGIN_PX
